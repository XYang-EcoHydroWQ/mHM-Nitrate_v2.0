...
CONTAINS
  ...
  FUNCTION BIAS_dp_1d(x, y, mask)
    ...
    BIAS_dp_1d = 100.0_dp * sum((y-x), mask = maske)/sum(x, mask = maske)
  ...
  FUNCTION SSE_dp_1d(x, y, mask)
    ...
    real(dp),dimension(size(x))        :: weight  !
    ...
    !only for yangx 2016-10-24
    !modified by yangx 2016-11-21
    where (x > 5.0_dp) 
       weight = 3.0_dp
    elsewhere
       weight = 1.0_dp
    end where
    SSE_dp_1d = sum(weight * abs(y - x) ,mask = maske) / sum(weight, mask = maske)
