...
CONTAINS
  ...
  subroutine mRM_routing( &
	...
	!write(122,*) L11_Qmod(208)
	!write(123,*) L11_Qmod(283)
...
!    end do
    do gg = 1, nGauges
       if (gaugeNodeList(gg) .ne. nodata_i4) then
  ...
  !                  Xiaoqiang Yang	  Sep 2018 - a simple routing method for 1km resolution
  ! ------------------------------------------------------------------
  subroutine L11_routing( &
    ...
    do k = 1 , nLinks
       ...
       ! routing iNode
	   !corresponding to routing method changed to simple delayed coeff.: c1= param1,c2=0
	   !
       netNode_qTR(iNode,IT) = netNode_qTR(iNode,IT1)                               &
            + netLink_C1(i) * ( netNode_qTIN(iNode,IT) - netNode_qTR (iNode,IT1) ) 
       ...
       if (nInflowGauges .GT. 0) then
          do i = 1, nInflowGauges
             ...
             if ( (tNode == InflowNodeList(i)) .AND. ( InflowHeadwater(i))) netNode_qTR(iNode,IT) = 0.0_dp
          end do
       end if
  ...
  SUBROUTINE L11_runoff_acc(qAll, efecArea, L1_L11_Id, &
    ...
    logical,     dimension(:), intent(in)  :: InflowHeadwater ! [-]        if to consider headwater cells of inflow gauge
    integer(i4), dimension(:), intent(in)  :: InflowNodeList  ! [-]        L11 ID of inflow points
    real(dp),    dimension(:), intent(inout)  :: QInflow         ! [m3 s-1]   inflowing water
    ...
    if (nInflowGauges .gt. 0) then
       where (abs(QInflow - nodata_dp) < 0.1_dp ) QInflow = 0.0_dp  !added by yangx for inflow from point source input
       do k = 1, nInflowGauges
          if (.not. InflowHeadwater(k)) then 
    ...
    !write(40,*) InflowNodeList,InflowIndexList,QInflow(InflowIndexList),qout(:)!
