...
CONTAINS
  ...
  FUNCTION objective_sse(parameterset)
    ...
    use mo_errormeasures,    only: sse, rmse
    ...
    real(dp)              :: average_q, average_in

    call eval(parameterset, runoff=runoff, nutrient = nutrient)
    nGaugesTotal = size(runoff, dim=2)

    !open(222, file='extract_in.txt', position = 'append')
    ...
    do gg=1, nGaugesTotal
       ...
       average_q = sum(runoff_obs, runoff_obs_mask)/max(1, count(runoff_obs_mask))
       average_in = sum(nitrate_obs, nitrate_obs_mask)/max(1,count(runoff_obs_mask))
       !if (gg == 1) then
	   !write(222,*) average_q, average_in
       !write(222,*) gg
	   !write(222,*) nitrate_agg(:)
      ! write(222,*) nitrate_obs(:)
	   !end if
	   ! MULTI-objective
       !objective_sse = objective_sse + &
       !     (rmse( runoff_obs/average_q, runoff_agg/average_q, mask=runoff_obs_mask) + & 
	   !	rmse( nitrate_obs/average_in, nitrate_agg/average_in, mask=nitrate_obs_mask))/2.0_dp
       ! calibration for discharge/nitrate
       !objective_sse = objective_sse + &
       !              sse(runoff_obs,runoff_agg, mask= runoff_obs_mask)
       objective_sse = objective_sse + &
                       rmse(nitrate_obs,nitrate_agg, mask= nitrate_obs_mask)
   end do
  ...
  subroutine extract_nitrate( gaugeId, nitrate, nitrate_agg, nitrate_obs, nitrate_obs_mask )
    ...
    use mo_mrm_global_variables, only: gauge,  evalPer, warmingDays_mrm, nTstepDay!nMeasPerDay,
  ...
  ! ------------------------------------------------------------------
  subroutine eval(parameterset, runoff, nutrient, basin_avg_tws)
...
#ifdef mrm2mhm
    call mHM_eval(parameterset, runoff=runoff, nutrient= nutrient, basin_avg_tws=basin_avg_tws)
#else
    call mRM_eval(parameterset, runoff=runoff, nutrient=nutrient)
#endif
