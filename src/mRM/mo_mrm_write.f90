...
! 302 format (a7, i2, a32,es20.8)
  ...
  subroutine write_daily_obs_sim_discharge(Qobs, Qsim)
    ...
    use mo_errormeasures, only: kge, nse, bias,rmse
    ...
    do bb = 1, nBasins
       ...
       do gg=igauge_start, igauge_end
          if (count(ge(Qobs(:,gg), 0.0_dp)) > 1 )  then
             call message('    RMSE of daily discharge (gauge #',trim(adjustl(num2str(gg))),'): ', &
                  trim(adjustl(num2str(rmse(Qobs(:,gg), Qsim(:,gg), mask=(ge(Qobs(:,gg), 0.0_dp)))))) )
             ...
             call message('    PBIAS(%) of daily discharge (gauge #',trim(adjustl(num2str(gg))),'): ', &
                  trim(adjustl(num2str(bias(Qobs(:,gg), Qsim(:,gg), mask=(ge(Qobs(:,gg), 0.0_dp)))))) )
          end if
       end do
